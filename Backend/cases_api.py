...
from database import get_cases_for_user, add_user_to_case, get_investigators_details, remove_user_from_case
...
def api_delete_case():
    ...
    if not case_id:
        return jsonify({"ok": False, "error": "Case ID is required"}), 400
    ...
    try:
        ...
        return jsonify({"ok": True, "message": "Case deleted successfully"}), 200
...
def api_remove_user_from_case():
    ...
    if success:
        ...
        return jsonify({
            "ok": False, 
            "error": "لا يمكن حذف مالك القضية أو أن المحقق غير موجود"
        }), 400
